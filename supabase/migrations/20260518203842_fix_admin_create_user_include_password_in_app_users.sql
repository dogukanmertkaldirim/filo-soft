/*
  # Fix admin_create_user - Include password in app_users INSERT

  1. Problem
    - The login system checks `app_users.password` directly
    - The `admin_create_user` RPC was not inserting into the `password` column
    - This causes: "null value in column password of relation app_users violates not-null constraint"

  2. Fix
    - Add `password` to the INSERT statement for `app_users`
    - Store the plaintext password (matching the existing login system behavior)

  3. No Other Changes
    - Same function signature and grants
...
BEGIN
  ...
  INSERT INTO app_users (
    id, username, password, email, full_name, phone, title, role, company_id,
    linked_customer_id, assigned_rep_id, driver_license_no, driver_license_expiry,
    is_active, created_at, updated_at
  ) VALUES (
    v_user_id, p_username, p_password, v_email, p_full_name,
    NULLIF(trim(p_phone), ''), NULLIF(trim(p_title), ''), p_role, p_company_id,
    p_linked_customer_id, p_assigned_rep_id,
    NULLIF(trim(p_driver_license_no), ''), p_driver_license_expiry,
    true, now(), now()
  );
