/*
  # Fix admin_create_user RPC Function
  
  Drop the old overloaded version and create a single clean version.
*/

DROP FUNCTION IF EXISTS admin_create_user(text, text, text, text, text, text, text, uuid, uuid, text, date);
DROP FUNCTION IF EXISTS admin_create_user(text, text, text, text, text, text, text, uuid, uuid, uuid, text, date);
