/*
  # Fix admin_create_user RPC - Schema-qualify pgcrypto functions

  1. Problem
    - The `admin_create_user` function uses `SET search_path = public`
    - `gen_salt` and `crypt` live in the `extensions` schema
    - Calling `gen_salt('bf')` without schema prefix causes:
      "function gen_salt(unknown) does not exist"

  2. Fix
    - Replace `crypt(...)` with `extensions.crypt(...)`
    - Replace `gen_salt(...)` with `extensions.gen_salt(...)`

  3. Affected Operations
    - User creation (Kullanici Ekle)
    - Customer account creation (Musteri Ekle)
    - Driver account creation (Surucu Ekle)

  4. Security
    - No RLS changes
    - Function remains SECURITY DEFINER
    - Grants remain for authenticated and anon roles
*/
